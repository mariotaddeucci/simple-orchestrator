...
class McpLocalConfig(BaseModel):
    ...
    def _validate_import_path(self) -> "McpLocalConfig":
        if ":" not in self.import_path:
            raise ValueError(
                f"import_path {self.import_path!r} must be in 'module:attribute' format, e.g. 'my_tools.server:mcp'"
            )
        return self
    ...
    def to_stdio(self) -> McpStdioConfig:
        ...
        script = (
            "import importlib; "
            f"_mod, _attr = {self.import_path!r}.rsplit(':', 1); "
            "getattr(importlib.import_module(_mod), _attr).run(transport='stdio')"
        )
