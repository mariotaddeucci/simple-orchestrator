...
@pytest.mark.anyio
async def test_tui_create_edit_delete_agent(tmp_path) -> None:
    db = OrchestratorDB(tmp_path / "tui_agents.db")
    db.connect()
    ...
    async with app.run_test(size=(140, 50)) as pilot:
        ...
        await wait_until(pilot, lambda: db.get_agent(agent_id) is not None)
        app.action_refresh()
        ...
        await wait_until(
            pilot,
            lambda: (a := db.get_agent(agent_id)) is not None and a.name == "Edited Agent",
        )
        app.action_refresh()
