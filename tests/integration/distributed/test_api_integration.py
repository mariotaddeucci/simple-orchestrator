...
@pytest.mark.anyio
async def test_api_enqueue_and_worker_execution_success(orch_db, api_client):
    ...
    lease = await api_client.dequeue_next()
    assert lease is not None
    await runner._process_lease(lease)

    completed = await api_client.get_queue_item(item.id)
    ...
    session = await api_client.get_session(completed.session_id)
...
@pytest.mark.anyio
async def test_api_enqueue_and_worker_execution_failure(orch_db, api_client):
    pytest.skip("FIXME: Failing with 422 in update_queue_item")
    await api_client.upsert_agent(
        AgentUpsertRequest(
            id="mock-test-agent",
    ...
    item = await api_client.enqueue(EnqueueRequest(agent_id="mock-test-agent", prompt="This prompt will fail"))
    ...
    failed = await api_client.get_queue_item(item.id)
