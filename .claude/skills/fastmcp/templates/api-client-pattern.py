...
async def retry_with_backoff(
    func, max_retries: int = 3, initial_delay: float = 1.0, exponential_base: float = 2.0
):
...
@mcp.tool()
async def api_get(endpoint: str, use_cache: bool = True) -> dict:
    """
...
@mcp.tool()
async def api_post(endpoint: str, data: dict, invalidate_cache: bool = True) -> dict:
    """
...
@mcp.tool()
async def api_put(endpoint: str, data: dict, invalidate_cache: bool = True) -> dict:
...
@mcp.tool()
async def api_delete(endpoint: str, invalidate_cache: bool = True) -> dict:
    ...
    try:
        ...
        return {"success": True, "status_code": status, "deleted": True}
...
@mcp.tool()
async def batch_api_requests(endpoints: list[str], use_cache: bool = True) -> dict:
    """
...
async def api_status() -> dict:
    ...
    try:
        ...
        return {"api_reachable": False, "error": str(e), "timestamp": datetime.now().isoformat()}
