...
def format_error(error: str, code: str = "ERROR") -> Dict[str, Any]:
    ...
    return {"success": False, "error": error, "code": code, "timestamp": datetime.now().isoformat()}
...
@mcp.tool()
async def process_data(data: List[Dict[str, Any]], operation: str = "summarize") -> dict:
    """
    ...
    try:
        if operation == "summarize":
            ...
            return format_success(
                {"original_count": len(data), "filtered_count": len(filtered), "data": filtered}
            )

        elif operation == "transform":
            transformed = [{**d, "processed_at": datetime.now().isoformat()} for d in data]
...
@mcp.tool()
async def fetch_from_api(endpoint: str, use_cache: bool = True) -> dict:
    """
    ...
    try:
        ...
        return format_error(f"HTTP {e.response.status_code}", "HTTP_ERROR")
    except Exception as e:
...
def calculate(operation: str, a: float, b: float) -> dict:
    ...
    return format_success({"operation": operation, "a": a, "b": b, "result": result})


@mcp.tool()
async def batch_process_with_progress(items: List[str], context: Context) -> dict:
    """
    ...
    for i, item in enumerate(items):
        ...
        await context.report_progress(
            progress=i + 1, total=total, message=f"Processing {i + 1}/{total}: {item}"
        )

        # Simulate processing
        await asyncio.sleep(0.1)
        results.append({"item": item, "processed": item.upper(), "index": i})

    return format_success({"total": total, "results": results}, "Batch processing complete")
