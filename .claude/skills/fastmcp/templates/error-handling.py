...
def divide_numbers(a: float, b: float) -> dict:
    ...
    try:
        if b == 0:
            return create_error(
                ErrorCode.VALIDATION_ERROR, "Division by zero is not allowed", {"a": a, "b": b}
            )

        result = a / b
        return create_success({"result": result, "a": a, "b": b}, "Division successful")

    except Exception as e:
        return create_error(ErrorCode.UNKNOWN_ERROR, f"Unexpected error: {str(e)}")
...
async def validated_operation(data: str, min_length: int = 1) -> dict:
    ...
    try:
        ...
        return create_success(
            {"original": data, "processed": processed}, "Data processed successfully"
        )
    except Exception as e:
        return create_error(ErrorCode.UNKNOWN_ERROR, str(e))
...
async def resilient_api_call(url: str) -> dict:
    ...
    try:
        ...
        return create_success(data, "API call successful")

    except httpx.TimeoutException:
        return create_error(
            ErrorCode.TIMEOUT, "Request timed out", {"url": url, "timeout_seconds": 10}
        )

    except httpx.NetworkError as e:
        return create_error(
            ErrorCode.NETWORK_ERROR, "Network error occurred", {"url": url, "error": str(e)}
        )

    except httpx.HTTPStatusError as e:
        if e.response.status_code == 404:
            return create_error(
                ErrorCode.NOT_FOUND, "Resource not found", {"url": url, "status_code": 404}
            )
        elif e.response.status_code == 401:
            return create_error(
                ErrorCode.UNAUTHORIZED, "Unauthorized access", {"url": url, "status_code": 401}
            )
        elif e.response.status_code == 429:
            return create_error(
                ErrorCode.RATE_LIMITED, "Rate limit exceeded", {"url": url, "status_code": 429}
            )
        else:
            return create_error(
                ErrorCode.API_ERROR,
        ...
        return create_error(ErrorCode.UNKNOWN_ERROR, f"Unexpected error: {str(e)}", {"url": url})
...
async def batch_with_error_recovery(items: list[str]) -> dict:
    ...
    for i, item in enumerate(items):
        try:
            ...
            results.append({"index": i, "item": item, "processed": item.upper(), "success": True})
            ...
            errors.append(
                {"index": i, "item": item, "error": str(e), "code": ErrorCode.UNKNOWN_ERROR.value}
            )
...
async def safe_database_operation(query: str) -> dict:
    ...
    try:
        ...
        if "DROP" in query.upper():
            return create_error(
                ErrorCode.UNAUTHORIZED, "DROP operations not allowed", {"query": query}
            )

        if not query.strip():
            return create_error(ErrorCode.VALIDATION_ERROR, "Query cannot be empty")
        ...
        return create_success(
            {"rows": mock_data, "count": len(mock_data)}, "Query executed successfully"
        )

    except Exception as e:
        return create_error(ErrorCode.API_ERROR, f"Database error: {str(e)}", {"query": query})
...
async def detailed_health_check() -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            ...
            checks["api"] = {"status": "healthy", "status_code": response.status_code}
    except Exception as e:
