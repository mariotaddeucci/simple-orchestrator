...
@mcp.tool()
def process_text(
    text: str, uppercase: bool = False, prefix: Optional[str] = None, suffix: Optional[str] = None
) -> str:
...
async def batch_process(items: List[str]) -> Dict[str, Any]:
    ...
    for i, item in enumerate(items):
        try:
            ...
            errors.append({"index": i, "item": item, "error": str(e), "success": False})
...
async def safe_operation(data: dict) -> dict:
    ...
    try:
        # Validate input
        if not data:
            return {"success": False, "error": "Data is required", "code": "VALIDATION_ERROR"}

        # Simulate operation
        await asyncio.sleep(0.1)
        processed_data = {k: v.upper() if isinstance(v, str) else v for k, v in data.items()}

        return {"success": True, "data": processed_data, "timestamp": datetime.now().isoformat()}

    except KeyError as e:
        return {"success": False, "error": f"Missing key: {e}", "code": "KEY_ERROR"}
    except Exception as e:
...
async def tool_with_progress(count: int, context: Context) -> dict:
    ...
    for i in range(count):
        ...
        if context and hasattr(context, "report_progress"):
            await context.report_progress(
                progress=i + 1, total=count, message=f"Processing item {i + 1}/{count}"
            )
    ...
    return {"count": count, "results": results, "status": "completed"}
...
async def tool_with_elicitation(context: Context) -> dict:
    ...
    if context and hasattr(context, "request_elicitation"):
        ...
        user_name = await context.request_elicitation(
            prompt="What is your name?", response_type=str
        )

        return {"greeting": f"Hello, {user_name}!", "timestamp": datetime.now().isoformat()}
    else:
