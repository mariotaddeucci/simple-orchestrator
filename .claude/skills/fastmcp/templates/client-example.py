...
async def test_tools_examples():
    ...
    async with Client("tools-examples.py") as client:
        ...
        result = await client.call_tool(
            "validated_search", {"params": {"query": "python", "limit": 5}}
        )
        print(f"Search result: {result.data}")

        # Test batch processing
        print("\nTesting batch process...")
        result = await client.call_tool("batch_process", {"items": ["item1", "item2", "item3"]})
...
async def test_with_error_handling():
    ...
    async with Client("error-handling.py") as client:
        ...
        try:
            result = await client.call_tool("divide_numbers", {"a": 10, "b": 2})
        ...
        try:
            result = await client.call_tool("validated_operation", {"data": ""})
            print(f"Result: {result.data}")
        except Exception as e:
            print(f"Error: {e}")
